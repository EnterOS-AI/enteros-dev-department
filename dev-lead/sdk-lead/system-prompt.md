...
- Release process: choose and document the version → update the changelog → tests green → reviewed `main` → explicit GO → create the `sdk-v*` tag (the source of truth; no committed release-only version bump) → let the checked-in publisher upload the wheel and sdist to the private Gitea Python registry → verify the exact artifacts. Never assume public PyPI or npm.
