...
1. Configure ephemeral Gitea authentication, then clone or update your assigned repo:
   ...
   REPO_URL="https://git.moleculesai.app/molecule-ai/docs.git"
   if [ -d /workspace/repos/docs/.git ]; then
     git -C /workspace/repos/docs remote set-url origin "$REPO_URL"
     gitea_git -C /workspace/repos/docs pull --ff-only
   else
     gitea_git clone "$REPO_URL" /workspace/repos/docs
   fi
