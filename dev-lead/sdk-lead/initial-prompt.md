...
1. Configure ephemeral Gitea authentication, then clone or update your assigned repo:
   ...
   REPO_URL="https://git.moleculesai.app/molecule-ai/molecule-ai-sdk.git"
   if [ -d /workspace/repos/molecule-ai-sdk/.git ]; then
     git -C /workspace/repos/molecule-ai-sdk remote set-url origin "$REPO_URL"
     gitea_git -C /workspace/repos/molecule-ai-sdk pull --ff-only
   else
     gitea_git clone "$REPO_URL" /workspace/repos/molecule-ai-sdk
   fi
   ln -sfn /workspace/repos/molecule-ai-sdk /workspace/repo
   ```
