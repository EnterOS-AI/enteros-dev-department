...
1. Configure ephemeral Gitea authentication, then clone or update your assigned repo:
   ...
   if [ -d /workspace/repos/molecule-core/.git ]; then
     git -C /workspace/repos/molecule-core remote set-url origin "$REPO_URL"
     gitea_git -C /workspace/repos/molecule-core pull --ff-only
   else
     gitea_git clone "$REPO_URL" /workspace/repos/molecule-core
   fi
