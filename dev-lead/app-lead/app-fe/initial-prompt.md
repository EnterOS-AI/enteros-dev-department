...
1. Configure ephemeral Gitea authentication, then clone or update your assigned repo:
   ...
   REPO_URL="https://git.moleculesai.app/molecule-ai/molecule-app.git"
   if [ -d /workspace/repos/molecule-app/.git ]; then
     git -C /workspace/repos/molecule-app remote set-url origin "$REPO_URL"
     gitea_git -C /workspace/repos/molecule-app pull --ff-only
   else
     gitea_git clone "$REPO_URL" /workspace/repos/molecule-app
   fi
